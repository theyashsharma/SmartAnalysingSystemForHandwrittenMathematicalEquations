...
def checkSolution(request):
    ...
    def getBestShift(img):
        ...
        rows,cols = img.shape
        #rounding off reduced array and converting the array values in int to find the shift
    ...
    def predict(img,x1,y1,x2,y2, proba = False, acc_thresh = 0.60):
        ...
        #dilate fn increases the object area and make feaures more prominent or noticable
        ...
        if(len(contours_tmp) > 1): #if number of contours > 1
            # Find the contours
            ...
            max_index = np.argmax(areas) #return index of maximum area
            cnt=contours[max_index] #assigning contour with max index
    ...
    def process_img (gray, resize_flag = 1, preproc = 0):
        ...
        if (preproc == 0):
            ...
            gray = cv2.erode(gray, kernel,iterations=2) #to erode the boundary of foreground object  
        ...
        if(resize_flag) == 1:
            interpolation=cv2.INTER_AREA #to calculate pixel values for new image
        else:
            interpolation=cv2.INTER_CUBIC #to calculate pixel values for new image
    ...
    for bn in box_nums:
        ...
        outputObjName = str(bn) + fileObj.name
        path = './outputMedia/'
        plt.savefig(path + outputObjName)

        final_path1 = path + str(0) + fileObj.name 
        final_path2 = path + str(1) + fileObj.name 
        final_path3 = path + str(2) + fileObj.name  


    context={'filePathName':filePathName, 'outputPathName1': final_path1, 'outputPathName2': final_path2, 'outputPathName3': final_path3}
    return render(request, 'index.html', context)